...
grep -F 'egress-policy) egress_policy' "$overlay"
grep -Fx 'EGRESS_FILTER=/etc/openhands/egress-filter' "$overlay"

egress_nft="$repo_root/worker/rootfs/etc/openhands/egress.nft"
egress_filter="$repo_root/worker/rootfs/etc/openhands/egress-filter"
tinyproxy_conf="$repo_root/worker/rootfs/etc/tinyproxy/tinyproxy.conf"
egress_unit="$repo_root/worker/rootfs/etc/systemd/system/openhands-egress.service"
grep -F 'meta skuid != "agent" accept' "$egress_nft"
grep -F 'counter drop' "$egress_nft"
grep -Fx 'FilterDefaultDeny Yes' "$tinyproxy_conf"
grep -Fx 'Filter "/etc/openhands/egress-filter"' "$tinyproxy_conf"
grep -Fx 'Listen 127.0.0.1' "$tinyproxy_conf"
grep -Fx 'ExecStart=/usr/sbin/nft -f /etc/openhands/egress.nft' "$egress_unit"
grep -Fx 'Before=agent-canvas.service' "$egress_unit"
grep -Fx 'Environment=HTTPS_PROXY=http://127.0.0.1:8888' "$unit"
grep -F 'Environment=NO_PROXY=' "$unit" | grep -F '.h-cloud.lan'
if grep -Eq '^[^#]*\^\.\*\$' "$egress_filter"; then echo 'egress filter must not allow every host'; exit 1; fi
