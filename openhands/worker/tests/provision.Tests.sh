...
setup_fixture() {
  ...
  printf '%s\n' \
    '#!/bin/sh' \
    'if test "${1:-}" = --version; then printf "%s\\n" "omni version v0.10.16 (fixture)"; exit 0; fi' \
  ...
  printf '%s\n' \
    ...
    '  https://github.com/lkshrk/omni/releases/download/v0.10.16/checksums.txt)' \
    ...
    '  https://github.com/lkshrk/omni/releases/download/v0.10.16/omni_linux_*.tar.gz)' \
    '    case "$(cat /tmp/fixture-machine-arch)" in x86_64|amd64) omni_target=x86_64 ;; aarch64|arm64) omni_target=arm64 ;; *) exit 72 ;; esac' \
    '    test "$url" = "https://github.com/lkshrk/omni/releases/download/v0.10.16/omni_linux_$omni_target.tar.gz" || exit 72' \
