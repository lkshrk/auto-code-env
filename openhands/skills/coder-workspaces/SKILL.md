...
2. Identify the required repository, stacks, features and location. Every workspace is a Kubernetes pod on the `dev` template. The default `location=cluster` keeps the home on Ceph and prefers the desktop node k8s-12 whenever it is up, falling back to the cluster nodes when it is off; nothing to choose. `location=desktop` (preset "Desktop (towerr)") pins the workspace to k8s-12 with node-local storage, unavailable while the desktop is off; pick it only when the user asks for it or the work is I/O-bound such as `wow-sync --watch`. Never silently substitute local development.
...
An agent log line `missing logical tool "<name>"` from `omni` means the workspace's preserved `~/dotfiles` checkout predates the template's tool catalog. Report it and ask the operator to fast-forward that checkout and restart; do not recreate the workspace or reset the checkout.
